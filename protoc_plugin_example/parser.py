...
class CodeGeneratorParser(object):
    ...
    def add_structure(self,
            ...
            descriptor,
            prefix: str = ""):
        structures["messages"][prefix + descriptor.name] = self.generate_structure(descriptor)

        for descriptor in descriptor.nested_type:
            self.add_structure(structures, descriptor, prefix="%s." % descriptor.name)

        for descriptor in descriptor.enum_type:
            self.add_enum(structures, descriptor, prefix="%s." % descriptor.name)

    def generate_structure(self, descriptor):
        structure = {
        ...
        for field in descriptor.field:
            structure["fields"] = self.generate_field(field)

        return structure

    def generate_field(self, descriptor):
        return {
            "name": descriptor.name,
            "type": FieldDescriptorProto.Type.Name(descriptor.type),
            "label": FieldDescriptorProto.Label.Name(descriptor.label),
            "type_name": descriptor.type_name,
            "extendee": descriptor.extendee,
            "default_value": descriptor.default_value,
            "oneof_index": descriptor.oneof_index,
            "json_name": descriptor.json_name,
            "options": self.generate_options(descriptor.options),
        }

    def generate_options(self, descriptor):
        return {
    ...
    def add_enum(self,
            ...
            descriptor,
            prefix: str = ""):
        structures["enums"][prefix + descriptor.name] = { }
